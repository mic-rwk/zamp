                                    ...
                                    )
{
  ...
  if(strcmp(sElemName,"Lib") == 0) {
    char * name = xercesc::XMLString::transcode(rAttrs.getValue(
      xercesc::XMLString::transcode("Name")));
    config.AddPlugin(name);
    xercesc::XMLString::release(&name);
  } else if(strcmp(sElemName, "Cube") == 0) {
      Object3D object;
      auto getAttr = [&](const char * key, std::string & target) {
        char * value = xercesc::XMLString::transcode(rAttrs.getValue(
          xercesc::XMLString::transcode(key)));
        if (value) {
          target = value;
      ...
      getAttr("Name", object.name);
      getAttr("Shift", object.shift);
      getAttr("Scale", object.scale);
      getAttr("RotXYZ_deg", object.rot);
      getAttr("Trans_m", object.trans);
      getAttr("RGB", object.rgb);
