...
bool ProgramInterpreter::ExecProgram(const char *fileName_Prog)
{
    ...
    try {
        preprocessed = RunPreprocessor(fileName_Prog);
    ...
    for (const auto &p : _xmlConfig.plugins) {
        std::string fullPath = "lib/" + p.libName;
...
bool ProgramInterpreter::Read_XML_Config(const char *fileName) {
   ...
   std::filesystem::path configPath = std::filesystem::absolute(fileName).parent_path() / "config.xsd";

    if (!std::filesystem::exists(configPath)) {
        std::cerr << "!!! Plik gramatyki nie istnieje: " 
                  << configPath.string() << "\n";
        return false;
    }
   ...
   try {
     ...
     if (!pParser->loadGrammar(configPath.string().c_str(),
