...
void LoadPlugin(){
  std::vector<std::string> pluginNames = {
          "./lib/libInterp4Set.so",
          "./lib/libInterp4Rotate.so",
          "./lib/libInterp4Pause.so",
          "./lib/libInterp4Move.so"
      };
...
int main(int argc, char **argv)
{
    ...
    if (argc > 2) {
        ...
        std::cerr << "Błąd: należy podać plik XML z konfiguracją oraz plik z komendami.\n";
        return 1;
    }
    ...
    if (channel.GetSocket() < 0) {
        ...
        if (!program.ExecProgram(commandFile)) {
